...
@section('content')
    ...
    <div class="container">
        ...
        <form style="margin:auto;" action="{{ route('question.update', $question) }}" method="post">
            ...
            <div class="parameter" style="display: none;">
                ...
                <input name="tags" type="text" id="hidden-tags-form" class="form-control" placeholder="e.g.(Java, VPN, exampleTag)" value="{{$tags}}">
            </div>
            <div style="display: none;">
                <button type="submit" class="btn btn-primary mt-1" id="hidden-submit-btn" >Update Question</button>
            </div>
    ...
    <script>
        ...
        let maxTags = 5,
        tags = []
        let temp2 = document.getElementById("hidden-tags-form").value
        tags = JSON.parse(temp2).map(e => e.name)
        countTags();
        createTag();
        function countTags() {
            input.focus();
